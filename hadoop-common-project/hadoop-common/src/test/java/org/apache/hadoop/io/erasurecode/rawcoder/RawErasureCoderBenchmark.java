...
public final class RawErasureCoderBenchmark {
  ...
  private static final int TARGET_BUFFER_SIZE_MB = 1024;
  ...
  private static ByteBuffer genTestData(boolean useDirectBuffer, int sizeKB) {
    ...
    long memAccessStart = System.nanoTime();
    data.put(tmp);
    data.flip();
    long memAccessEnd = System.nanoTime();
    System.out.println("zmdebug: sizeKB=" + sizeKB + ", memoryAccessTime(ms)= "+ (memAccessEnd - memAccessStart)/1000000.);
    return data;
  }

  private static class BenchData {
    public static final ErasureCoderOptions OPTIONS =
        new ErasureCoderOptions(4, 1);
    ...
    private static final int[] ERASED_INDEXES = new int[]{0};//POCache added
    ...
    public static void configure(int dataSizeMB, int chunkSizeKB) {
      System.out.println("zmdebug: (k,m)=(" + NUM_DATA_UNITS + "," + NUM_PARITY_UNITS +").");
      ...
      for (long i = 0; i < rounds; i++) {
        while (testData.remaining() > 0) {
          ...
          if (isEncode) {
            ...
            System.out.println("zmdebug: encodeTime(ms)= "+ (encodeEnd - encodeStart)/1000000.);
          } else {
            benchData.prepareDecInput();
            long decodeStart = System.nanoTime();
            benchData.decode(decoder);
            long decodeEnd = System.nanoTime();
            System.out.println("zmdebug: decodeTime(ms)= "+ (decodeEnd - decodeStart)/1000000.);
          }
        }
        testData.clear();
      }
