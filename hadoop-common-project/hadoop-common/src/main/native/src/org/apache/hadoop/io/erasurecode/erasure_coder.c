...
static void initEncodeMatrix(int numDataUnits, int numParityUnits,
  ...
  // Generate Vandermonde matrix for parity update
  // ZM modify on Nov. 21, 2017
  h_gf_gen_rs_matrix(encodeMatrix, numDataUnits + numParityUnits, numDataUnits);
}
...
// Refer to ISA-L implementation:
//  include/erasure_code.h
//  erasrue_code/erasure_code_update_test.c
// ZM add on Mar. 5, 2018
int encode_update(IsalEncoder* pCoder, unsigned char* dataUnit,
        unsigned char** parityUnits, int chunkSize, int vec_i) {
    /* printf("Enter erasure_coder - encode_update(): vec_i=%d, numDataUnits=%d, numParityUnits=%d\n", vec_i, numDataUnits, numParityUnits); */
    int numDataUnits = pCoder->coder.numDataUnits;
    int numParityUnits = pCoder->coder.numParityUnits;
    h_ec_encode_data_update(chunkSize, numDataUnits, numParityUnits, vec_i,
            pCoder->gftbls, dataUnit, parityUnits);
    return 0;
}
