...
public class DFSClient implements java.io.Closeable, RemotePeerFactory,
  ...
  // POCache added
  private boolean hedgedOn = false;// inidicate whether to use hedgedRead in normal reading
  ...
  public DirectBufferPool directBufferPool = new DirectBufferPool();

  // POCache added
  private boolean pcacheOn = false;// inidicate whether to cache parity in memory
  private boolean parallelReadOnly = false; // indicate whether only do parallel reading without ec
  private static ThreadPoolExecutor PCACHE_READ_THREAD_POOL;
  private long subblkSize;//SJadd for subblock size
  private int numThreadsForWriteSubBlk;//SJadd threads for writing sub blocks
  ...
  public DFSClient(URI nameNodeUri, ClientProtocol rpcNamenode,
    ...
    // POCache added
    this.hedgedOn = (conf.get(DFS_HEDGED_ON_KEY) == null) ?
          false : conf.getBoolean(DFS_HEDGED_ON_KEY, DFS_HEDGED_ON_DEFAULT);
    this.pcacheOn = (conf.get(DFS_PCACHE_ON_KEY) == null) ?
          false : conf.getBoolean(DFS_PCACHE_ON_KEY, DFS_PCACHE_ON_DEFAULT);
    this.parallelReadOnly = (conf.get(DFS_PARALLEL_READ_KEY) == null) ?
        false : conf.getBoolean(DFS_PARALLEL_READ_KEY, DFS_PARALLEL_READ_DEFAULT);
    this.subblkSize = dfsClientConf.getSubblkSize();
  ...
  // POCache added on Nov. 9, 2017
  // This is to get the value of pcacheOn
  public boolean isPcacheOn() {
    return this.pcacheOn;
  }

  public boolean isParallelReadOnly() {
    return this.parallelReadOnly;
  }

  // POCache added on Nov. 9, 2017
  // This is to get the value of hedgedOn
  public boolean isHedgedOn() {
    return this.hedgedOn;
  }
  ...
  public void reportCompleteRead(long fileId, String slowDataNode) 
      throws IOException {
    try {
      namenode.reportCompleteRead(fileId, slowDataNode);
    } catch (IOException e) {
  ...
  private DFSInputStream openInternal(LocatedBlocks locatedBlocks, String src,
    ...
    if (locatedBlocks != null) {
      ...
      ParityCacheInfo pcacheInfo = locatedBlocks.getParityCacheInfo(); // POCache added
      if (ecPolicy != null) {
        return new DFSStripedInputStream(this, src, verifyChecksum, ecPolicy,
            locatedBlocks);
      }

      return new DFSInputStream(this, src, verifyChecksum, locatedBlocks, pcacheInfo); // POCache added
    } else {
  ...
  /**
   * POCache added
   * Create pcache reads thread pool, PCACHE_READ_THREAD_POOL, if
   * it does not already exist.
   * @param num Number of threads for pcache reads thread pool.
   * If zero, skip pcache reads thread pool creation.
   */
  private synchronized void initThreadsNumForPcacheReads(int num) {
    if (num <= 0 || PCACHE_READ_THREAD_POOL != null) return;
    PCACHE_READ_THREAD_POOL = new ThreadPoolExecutor(1, num, 60,
        TimeUnit.SECONDS, new SynchronousQueue<Runnable>(),
        new Daemon.DaemonFactory() {
          private final AtomicInteger threadIndex = new AtomicInteger(0);
          @Override
    ...
    PCACHE_READ_THREAD_POOL.allowCoreThreadTimeOut(true);
    LOG.debug("Using pcache reads; pool threads={}", num);
  }
  /**
  ...
  // POCache added
  ThreadPoolExecutor getPcacheReadsThreadPool() {
    return PCACHE_READ_THREAD_POOL;
  }
