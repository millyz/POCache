...
public class DFSStripedInputStream extends DFSInputStream {
  ...
  private RawErasureDecoder __decoder;
  ...
  DFSStripedInputStream(DFSClient dfsClient, String src,
    ...
    __decoder = CodecUtil.createRawDecoder(dfsClient.getConfiguration(),
  ...
  private boolean useDirectBuffer() {
    return __decoder.preferDirectBuffer();
  }
  ...
  // POCache added on May 25, 2018 - to parallelize createBlockReader
  BlockReaderInfo createBlockReaderInfoPcache(LocatedBlock block, long offsetInBlock,
    ...
    while (true) {
      ...
      if (reader != null) {
        return new BlockReaderInfo(reader, dnInfo.info, offsetInBlock);
      }
    }
    return null;
  }
  ...
  private void readOneStripe(CorruptedBlocks corruptedBlocks)
    ...
    for (AlignedStripe stripe : stripes) {
      ...
      StripeReader sreader = new StatefulStripeReader(stripe, ecPolicy, blks,
          blockReaders, corruptedBlocks, __decoder, this);
      if (this.dfsClient.isPcacheOn()) {
        int pcacheReadPolicy = dfsClient.getConf().getPcacheReadPolicy();
        if (pcacheReadPolicy == 1) {
          sreader.readStripePcacheReactive(String.valueOf(dfsClient.getFileInfo(src).getFileId()), 0,
                stripeIndex, jedisc);
        } else if (pcacheReadPolicy == 2) {
          sreader.readStripePcacheProactive(String.valueOf(dfsClient.getFileInfo(src).getFileId()), 0,
                stripeIndex, jedisc);
        } else {
          sreader.readStripe();
        }
    ...
    try {
      for (AlignedStripe stripe : stripes) {
        ...
        StripeReader preader = new PositionStripeReader(stripe, ecPolicy, blks,
            preaderInfos, corruptedBlocks, __decoder, this);
        try {
          preader.readStripe();
        } finally {
          preader.close();
