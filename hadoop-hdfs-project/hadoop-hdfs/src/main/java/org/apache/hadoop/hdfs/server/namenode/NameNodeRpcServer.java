...
public class NameNodeRpcServer implements NamenodeProtocols {
  ...
  //public void reportPerfToPcm(List<String> dnIDs, List<Double> perfs) {
    //namesystem.reportPerfToPcm(dnIDs, perfs);
  //}
  
  // POCache added
  //public void reportSlowDNsToPcm(List<String> slowDNs, List<String> normalDNs) {
    //namesystem.reportSlowDNsToPcm(slowDNs, normalDNs);
  //}

  // POCache added
  public void reportCompleteRead(long fileId, String slowDataNode) {
    namesystem.reportCompleteRead(fileId, slowDataNode);
  }
  ...
  public HdfsFileStatus create(String src, FsPermission masked,
    ...
    try {
      ...
      status = namesystem.startFile(src, perm, clientName, clientMachine,
          flag.get(), createParent, replication, blockSize, supportedVersions,
          ecPolicyName, parityCacheOn, cacheEntry != null);
    } finally {
      RetryCache.setState(cacheEntry, status != null, status);
    }
  ...
  public HeartbeatResponse sendHeartbeat(DatanodeRegistration nodeReg,
      ...
      @Nonnull double nodeLatency) throws IOException {
    checkNNStartup();
    verifyRequest(nodeReg);
    return namesystem.handleHeartbeat(nodeReg, report,
        dnCacheCapacity, dnCacheUsed, xceiverCount, xmitsInProgress,
        failedVolumes, volumeFailureSummary, requestFullBlockReportLease,
        slowPeers, slowDisks, nodeLatency);
  }
