...
// POCache added
import org.apache.hadoop.hdfs.protocol.proto.ClientNamenodeProtocolProtos.ReportPerfToPcmRequestProto;
import org.apache.hadoop.hdfs.protocol.proto.ClientNamenodeProtocolProtos.ReportPerfToPcmResponseProto;
import org.apache.hadoop.hdfs.protocol.proto.ClientNamenodeProtocolProtos.ReportSlowDNsToPcmRequestProto;
import org.apache.hadoop.hdfs.protocol.proto.ClientNamenodeProtocolProtos.ReportSlowDNsToPcmResponseProto;
/* */
...
import org.apache.hadoop.hdfs.protocol.proto.ClientNamenodeProtocolProtos.ReportCompleteReadRequestProto;
import org.apache.hadoop.hdfs.protocol.proto.ClientNamenodeProtocolProtos.ReportCompleteReadResponseProto;
...
public class ClientNamenodeProtocolServerSideTranslatorPB implements
  ...
  // POCache added on Mar. 21, 2018
  private static final ReportPerfToPcmResponseProto VOID_REP_PERF_TO_PCM_RESPONSE =
          ReportPerfToPcmResponseProto.newBuilder().build();

  // POCache added
  private static final ReportSlowDNsToPcmResponseProto VOID_REP_SLOW_TO_PCM_RESPONSE =
  ReportSlowDNsToPcmResponseProto.newBuilder().build();

  private static final ReportCompleteReadResponseProto VOID_REP_COMPLETE_READ_RESPONSE =
  ReportCompleteReadResponseProto.newBuilder().build();
  ...
  // POCache added on Mar. 21, 2018
  //public ReportPerfToPcmResponseProto reportPerfToPcm(RpcController controller, ReportPerfToPcmRequestProto req)
      //throws ServiceException {
    //try {
      //List<String> dnIDs = req.getDnIDsList();
      //List<Double> perfs = req.getPerfsList();
      //server.reportPerfToPcm(dnIDs, perfs);
      //return VOID_REP_PERF_TO_PCM_RESPONSE;
    //} catch (IOException e) {
  ...
  // POCache added
  //public ReportSlowDNsToPcmResponseProto reportSlowDNsToPcm(RpcController controller, ReportSlowDNsToPcmRequestProto req)
      //throws ServiceException {
    //try {
      //List<String> slowDNs = req.getSlowDNsList();
      //List<String> normalDNs = req.getNormalDNsList();
      //server.reportSlowDNsToPcm(slowDNs, normalDNs);
      //return VOID_REP_SLOW_TO_PCM_RESPONSE;
    //} catch (IOException e) {
      //throw new ServiceException(e);
    //}
  //}
  ...
  // POCache added
  public ReportCompleteReadResponseProto 
    reportCompleteRead(RpcController controller, ReportCompleteReadRequestProto req)
      throws ServiceException {
    try {
      server.reportCompleteRead(req.getFileId(), req.getSlowDataNode());
      return VOID_REP_COMPLETE_READ_RESPONSE;
    } catch (IOException e) {
      throw new ServiceException(e);
    }
  }
  ...
  public CreateResponseProto create(RpcController controller,
    ...
    try {
      ...
      HdfsFileStatus result = server.create(req.getSrc(),
          ...
          req.getEcPolicyName(), req.getParityCacheOn());//POCache added pcinfo on Dec. 11, 2017
