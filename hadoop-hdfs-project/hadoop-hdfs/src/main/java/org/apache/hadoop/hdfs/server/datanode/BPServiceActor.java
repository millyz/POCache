...
class BPServiceActor implements Runnable {
  ...
  HeartbeatResponse sendHeartBeat(boolean requestBlockReportLease)
    ...
    //final BlockLatencyReports blocksLatency =
        //(dn.isPcacheOn() == true) ?
        //BlockLatencyReports.create(dn.getMetrics().getBlockLatency()):
        //BlockLatencyReports.EMPTY_REPORT;
    //final double nodeServiceRate = (dn.isPcacheOn() == true) ?
        //dn.getMetrics().getNodeServiceRate() :
        //0.0;
    final double nodeServiceRate = dn.getMetrics().getNodeServiceRate();
