...
public class DataNodeMetrics {
  ...
  public DataNodeMetrics(String name, String sessionId, int[] intervals,
    ...
    // POCache added, Sep. 18, 2018
    blockStats = new HashMap<>();
  ...
  private class BlockStat {
    ...
    BlockStat(long timestamp, double agingFactor) {
      this.count = 0;
      this.latency = 0;
    ...
    public void update(double latency, double timestamp) {
      this.count = this.count * Math.pow(agingFactor, (timestamp - this.timestamp)) + 1.0;
      this.latency = this.latency * Math.pow(agingFactor, (timestamp - this.timestamp)) + latency;
    }

    public double query(long timestamp) {
      update(this.latency / this.count, timestamp);
      return this.latency / this.count;
    }
  };

  private long timestamp = 0;
  private HashMap<Long, BlockStat> blockStats;
  public void addBlockLatency(long blockId, long latency) {
    BlockStat blockStat = blockStats.get(blockId);
    if (blockStat == null) {
      blockStats.put(blockId, new BlockStat(this.timestamp, 0.99));
      blockStat = blockStats.get(blockId);
    }

    blockStat.update(latency, this.timestamp);
    this.timestamp += 1;
  }

  public HashMap<Long, Double> getBlockLatency() {
    HashMap<Long, Double> blocksLatency = new HashMap<>();
    for (Map.Entry<Long, BlockStat> entry : this.blockStats.entrySet()) {
      Long k = entry.getKey();
      blocksLatency.put(k, (double) entry.getValue().query(timestamp));
    }
    return blocksLatency;
  }

  private long totalTime = 0;
  private long totalCount = 0;
  private long totalBytes = 0;
  public void addNodeServiceStat(long numBytes, long time) {
    LOG.info("qpdebug: node service request stats, time {}, numbytes {}", time, numBytes);
    this.totalTime += time;
    this.totalCount += 1;
    this.totalBytes += numBytes;
  }

  public double getNodeServiceRate() {
    LOG.info("qpdebug: heart beat send current stat, time {}, numBytes {}, totalCounts {}", this.totalTime, this.totalBytes, totalCount);
    if (this.totalTime == 0.0) return 0.0;
    return (double) (this.totalBytes / this.totalTime);
  }
}
